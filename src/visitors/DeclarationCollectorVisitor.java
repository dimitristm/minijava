...
public class DeclarationCollectorVisitor extends StringRepresentationVisitor {
    ...
    public Map<ClassAndIdentifier, MethodInfo> getMethods() {
        return this.methods;
    }

    public Map<String, String> getClassesAndTheirParents() {
        return this.classesAndTheirParents;
    }
    ...
    public String visit(ClassDeclaration n) throws Exception {
        ...
        if (classesAndTheirParents.containsKey(currentClass)) {
            throw new SemanticCheckException("This class has already been declared elsewhere: " + currentClass);
        }
    ...
    public String visit(ClassExtendsDeclaration n) throws Exception {
        ...
        if (!classesAndTheirParents.containsKey(parentClass)) {
            throw new SemanticCheckException("Class " + currentClass + " tried to inherit from a class called "
                    + parentClass + ", but that class had not been declared yet or has no declaration at all");
        }
    ...
    public String visit(MethodDeclaration n) throws Exception {
        ...
        if (methodInfo.returnType().equals("void")) {
            throw new SemanticCheckException("A void method was declared. There are no void methods in this language.");
        }
        if (methods.containsKey(new ClassAndIdentifier(currentClass, methodName))) {
        ...
        while (currentClassToCheckForWrongOverload != null) {
            MethodInfo inheritedMethodInfo = methods
                    .get(new ClassAndIdentifier(currentClassToCheckForWrongOverload, methodName));
            if (inheritedMethodInfo != null) {
                if (inheritedMethodInfo.equals(methodInfo)) {
                    break;
                } else {
                    throw new SemanticCheckException(
                            "Incorrect overload for method " + methodName + " in class " + currentClass);
                }
            }
